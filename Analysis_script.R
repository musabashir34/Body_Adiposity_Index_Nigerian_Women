...
write_csv(desc_table, file.path(FIGURES_DIR, "table1_descriptive_stats.csv"))
...
fig1 <- wrap_plots(box_plots, ncol = 3) +
  plot_annotation(title = "Figure 1. Distribution of Key Variables by Menopausal Status",
                   theme = theme(plot.title = element_text(face = "bold", size = 15)))
ggsave(file.path(FIGURES_DIR, "Figure-1.png"), fig1,
...
write_csv(corr_table, file.path(FIGURES_DIR, "table2_correlations.csv"))
...
fig3 <- ggplot(df, aes(x = BAI, y = HbA1c, color = MenoGroup)) +
  ...
  labs(title = "Figure 3. BAI vs. HbA1c by Menopausal Status",
...
# ==============================================================================
# 4. REGRESSION MODELS (Table 3, Figure 4)
# ==============================================================================
cat("\n", strrep("=", 70), "\n")
...
write_csv(reg_results_df, file.path(FIGURES_DIR, "table3_regression_results.csv"))
...
fig4 <- (fig4a + fig4b) +
  plot_annotation(title = "Figure 4. Regression Model Performance Comparison (5-fold CV)",
...
# ==============================================================================
# 5. FEATURE IMPORTANCE: LASSO COEFFICIENTS + RF IMPURITY (Table 4, Figures 5-6)
# ==============================================================================
cat("\n", strrep("=", 70), "\n")
...
write_csv(importance_df, file.path(FIGURES_DIR, "table4_feature_importance.csv"))
...
fig5 <- importance_df %>%
  ...
  labs(title = "Figure 5. Random Forest Feature Importance (Regression)",
       x = NULL, y = "Importance (impurity)") +
  theme(plot.title = element_text(face = "bold"))
ggsave(file.path(FIGURES_DIR, "figure5_feature_importance.png"), fig5, width = 8, height = 6, dpi = 300)
cat(sprintf("Saved: %s/figure5_feature_importance.png\n", FIGURES_DIR))
...
fig6 <- importance_df %>%
  ...
  labs(title = "Figure 6. LASSO Regression Coefficients",
       subtitle = "Grey = eliminated by regularization", x = NULL, y = "Standardized Coefficient") +
  theme(plot.title = element_text(face = "bold", size = 13))
ggsave(file.path(FIGURES_DIR, "figure6_lasso_coefficients.png"), fig6, width = 9, height = 6, dpi = 300)
cat(sprintf("Saved: %s/figure6_lasso_coefficients.png\n", FIGURES_DIR))
fig7 = fig6 + fig5
ggsave(file.path(FIGURES_DIR, "Figure-5.png"), fig7,
       width = 13, height = 5, dpi = 300)
...
write_csv(subgroup_df, file.path(FIGURES_DIR, "table5_subgroup_results.csv"))
